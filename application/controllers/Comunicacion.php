...
class Comunicacion extends CI_Controller{
  ...
  function index()
  {
    $data['custom_error']= '';
    $data['mensaje']= '';
    $this->load->view('obras/comunicacion/obras_comunicacion',$data);
 ...
 function update() {
    ...
    if ($this->input->post('mit')) {
      ...
      redirect('comunicacion/edit'.$cod_obrasdatos);
    }
  ...
  function edit() {
    ...
    if ($kd == NULL) {
     ...
     $data1['cod_obrasdatos'] = $kd;
        ...
        $this->load->view('obras/comunicacion/edit', $data1);
        $this->load->view('templates/footer');
  ...
  function registrar(){
    ...
    if ($this->form_validation->run() == TRUE)
    {
      ...
      $ci = $this->session->userdata('ci');
      $obra  = $this->session->userdata('comunicacion');
      $datos = 	array(
        "cod_usuario" => $ci,
        "nombre" => $nombre,
  			"sigla" => $sigla,
