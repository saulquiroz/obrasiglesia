...
class Encuentros extends CI_Controller{
  ...
  function index()
  {
    $data['custom_error']= '';
    $data['mensaje']= '';
    $this->load->view('obras/encuentros/obras_encuentros',$data);
 ...
 function update() {
    ...
    if ($this->input->post('mit')) {
      ...
      redirect('encuentros/edit'.$cod_obrasdatos);
    }
  ...
  function edit() {
    ...
    if ($kd == NULL) {
     ...
     #$data1['jurisdiccion'] = $dt->jurisdiccion;
     ...
     $data1['cod_obrasdatos'] = $kd;
        ...
        $this->load->view('obras/encuentros/edit', $data1);
        $this->load->view('templates/footer');
  ...
  function registrar(){
    ...
    if ($this->form_validation->run() == TRUE)
    {
      ...
      $ci = $this->session->userdata('ci');
      $obra  = $this->session->userdata('encuentros');
      $datos = 	array(
        "cod_usuario" => $ci,
        "nombre" => $nombre,
  			"sigla" => $sigla,
